@extends('layouts.admin')

@section('content')
    <div class="section-body">
        <div class="row">
            <div class="col-12 col-md-6 col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb ">
                                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                                <li class="breadcrumb-item "><a href="{{ url('/admin/schedules') }}">Schedules</a></li>
                                <li class="breadcrumb-item active" aria-current="page">Show</li>
                            </ol>
                        </nav>
                    </div>
                    <div class="card-header"> <h4> Schedule {{ $schedule->id }} </h4>
                        <div class="card-header-form">
                          <a href="{{ url('/admin/schedules') }}" title="Back"><button class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</button></a>
                          <a href="{{ url('/admin/schedules/' . $schedule->id . '/edit') }}" title="Edit Schedule"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>

                          <form method="POST" action="{{ url('admin/schedules' . '/' . $schedule->id) }}" accept-charset="UTF-8" style="display:inline">
                              @method('DELETE')
                              @csrf
                              <button type="submit" class="btn btn-danger btn-sm" title="Delete Schedule" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</button>
                          </form>
                        </div>
                    </div>
                    <div class="card-body">

                        <div class="table-responsive">
                            <table class="table table">
                                <tbody>
                                    <tr>
                                        <th>ID</th><td>{{ $schedule->id }}</td>
                                    </tr>
                                    <tr><th> Day Uz </th><td> {{ $schedule->day_uz }} </td></tr>
                                    <tr><th> Day Ru </th><td> {{ $schedule->day_ru }} </td></tr>
                                    <tr><th> Day En </th><td> {{ $schedule->day_en }} </td></tr>
                                    <tr><th> Day Krill </th><td> {{ $schedule->day_kr }} </td></tr>
                                    <tr><th> Time </th><td> {{ $schedule->time }} </td></tr>
                                    <tr><th> Description Uz </th><td> {{ $schedule->description_uz }} </td></tr>
                                    <tr><th> Description Ru </th><td> {{ $schedule->description_ru }} </td></tr>
                                    <tr><th> Description En </th><td> {{ $schedule->description_en }} </td></tr>
                                    <tr><th> Description Kr </th><td> {{ $schedule->description_kr }} </td></tr>
                                </tbody>
                            </table>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
